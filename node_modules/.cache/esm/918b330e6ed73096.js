_6ed‍.x([["checkCollisions",()=>checkCollisions]]);function get_opposite(angle, length, x, y) {
...
function collide(ship1, ship2 = null, laser = null) {
    ...
    ship1_coords.push(new_position)
    for (let i = 1; i <= 3; i++) {
        if (i == 0) {
        ...
        for (const ship in ships) {
            if (laser.ship !== ships[ship].id) {
                if (collide(ships[ship], null, laser)) {
                    if (!bullets.has([laser.ship, ship])) {
                        console.log('COLLIDED')
                        bullets.add([laser.ship, ship])
                    }
                    return true
                }
            }
