let player,ship,laser,circleCollision;_7f0‍.x([["game",()=>game]]);_7f0‍.w("../shared/player",[["player",["player"],function(v){player=v}]]);_7f0‍.w("../shared/ship",[["ship",["ship"],function(v){ship=v}]]);_7f0‍.w("../shared/laser",[["laser",["laser"],function(v){laser=v}]]);_7f0‍.w("./circlecol",[["circleCollision",["circleCollision"],function(v){circleCollision=v}]]);
    ...
    addPlayer(socket, player_user, pair = null) {
        if(pair === null) {
            ...
            _7f0‍.g.console.log(this.pairs[pair_proper])
            const ship = this.ships[this.pairs[pair_proper].ship]
            for (let i = 0; i < 10; i++) {
                for (let j = 0; j < 10; j++) { 
                    if (ship.grid[i][j] === 1 && ship.playerGrid[i][j] === 0) {
                        this.players[socket.id] = new player(player_user, ship.id, i, j, pair_proper)
                        ship.addPlayer(socket.id, this.players[socket.id].position)
                        this.keys[socket.id] = {
                            left: false,
                            right: false,
                            up: false,
