_61f‍.x([["checkCollisions",()=>checkCollisions]]);function get_opposite(angle, length, x, y) {
...
function collide(ship1, ship2 = null, laser = null) {
    ...
    const l1 = {x: minx1, y: miny1, width: maxx1 - minx1, height: maxy1 - miny1}
    ...
    if (ship2 === null) {
        ...
        last_angle = laser.totalrotation
        for (let i = 1; i <= 3; i++) {
            if (i === 1) {
                ...
                const new_angle = last_angle + Math.PI / 2
                if (i == 2) {
                    laser_coords.push(get_opposite(new_angle, 10, laser_coords[i-1].x, laser_coords[i-1].y))
                }
                else {
       ...
       function checkCollisions(lasers, ships) {
